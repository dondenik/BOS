...
void kprint(char *message) {
  kprint_at(message, -1, -1, vga_attr(WHITE, BLACK));
}
void kprintln(char *message) {
  kprint(message);
  kprint_at("\n", -1, -1, vga_attr(WHITE, BLACK));
}

char vga_attr(char fore, char back) { return back << 4 | fore; }
...
int print_char(char c, int col, int row, char attr) {
  ...
  if (!attr)
    attr = vga_attr(WHITE, BLACK);

  /* Error control: print a red 'E' if the coords aren't right */
  if (col >= MAX_COLS || row >= MAX_ROWS) {
    vidmem[2 * (MAX_COLS) * (MAX_ROWS)-2] = 'E';
    vidmem[2 * (MAX_COLS) * (MAX_ROWS)-1] = vga_attr(RED, WHITE);
...
void clear_screen() {
  ...
  for (i = 0; i < screen_size; i++) {
    screen[i * 2] = ' ';
    screen[i * 2 + 1] = vga_attr(WHITE, BLACK);
  }
